...
def getitem(symbol: str) -> Optional[DataObjStr]:
    ...
    for url, inv in getinv().items():
        items = (o for o in inv.objects if symbol.split(".")[0] == o.name.split(".")[0])
        for item in items:
            return (url, item)

    for url, inv in getinv().items():
        items = (o for o in inv.objects if symbol in o.name.split("."))
        for item in items:
            return (url, item)
    return None, None
...
def getdoc(symbol: str, nparas: int = 1) -> tuple[str, Formatted, Url]:
    ...
    if not elems:
        return (symbol, Formatted("", ["No documentation available"]), url)
    ...
    clean_text = (dt_text.replace("\xB6", "").strip()).replace("\x0A", "<nl>")
    fixed_part = f"```py\n{clean_text}```"

    # fixed-width text (inline code / identifiers)
    for fwe in dd.select("span.pre, tt"):
        fwe.replace_with(doc.new_string("`{}`".format(fwe.text.strip())))
    ...
    for eme in dd.select("em, .em, .i"):
        eme.replace_with(doc.new_string("__**{}**__".format(eme.text.strip())))

    # other code blocks
    for blk in dd.select("pre"):
        blk.replace_with(
            doc.new_string(
                "```py\x0A{}```".format(blk.text.strip()).replace("\x0A", "<nl>")
            )
        )

    text = "\x0A".join(
        str(e) if isinstance(e, NavigableString) else e.text
        for e in (dd.children or dd)
    )
    paras = islice(text.split("\n\n\n\n"), 0, nparas)
    formatted_paras = []
    for para in paras:
        para2 = re.subn("\\s*\\n\\s*\\n", "<break>", para, re.DOTALL)[0]
    ...
    return (
        symbol,
        Formatted(fixed_part.replace("<nl>", "\x0A"), formatted_paras, elem),
        url,
    )
