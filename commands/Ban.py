...
class Ban(commands.Cog):
    ...
    async def ban(self, ctx: Context, member: Member, *, reason="No reason provided."):
        ...
        try:
            await member.send(
                f"You got banned from {ctx.guild.name} by {ctx.author} for reason: {reason}"
            )
        except:
    ...
    async def unban(
        self, ctx: Context, member: int = 0, *, reason="No reason provided."
    ):
        """Unban a User"""
        ...
        for ban_entry in banned_users:
            if member == ban_entry.user.id:
                ...
                await ctx.guild.unban(
                    int(member),
                    reason=f"Unbanned by {ctx.author.display_name} (id={ctx.author.id})",
                )
